...
environment = (os.getenv("ENVIRONMENT") or os.getenv("APP_ENV") or os.getenv("ENV") or "").lower()
is_production = environment in {"prod", "production"}

if not cors_origins_env:
    if is_production:
        print("⚠️  WARNING: CORS_ORIGINS не е зададен в production. Добавете домейна на frontend-а.")
        cors_origins_env = "https://astromind-web.onrender.com"
    else:
        cors_origins_env = "http://localhost:5173,http://127.0.0.1:5173,https://astromind-web.onrender.com"
...
# NOTE: Не инициализираме AI интерпретатора при import време.
# Това позволява бекендът да стартира без OPENAI_API_KEY/TOGETHER_API_KEY,
# като AI endpoints ще върнат смислена грешка вместо да "гърми" целият сървър.
...
async def interpret_chart_stream(request: ChartRequest):
    ...
    async def generate_monthly_stream():
        ...
        try:
            # Lazy init на AI интерпретатора (за да не блокира старта на сървъра).
            try:
...
async def interpret_chart(request: ChartRequest):
    ...
    try:
        ...
        # Missing/invalid API key should be surfaced as "service unavailable", not "bad input".
