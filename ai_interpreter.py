...
class AIInterpreter:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        """
        # Historical reason: some deployments stored the Together.ai key as OPENAI_API_KEY.
        # Prefer TOGETHER_API_KEY but keep OPENAI_API_KEY as a fallback for compatibility.
        self.api_key = api_key or os.getenv("TOGETHER_API_KEY") or os.getenv("OPENAI_API_KEY")
        
        if not self.api_key:
            raise ValueError(
                "API key не е намерен. Моля задайте TOGETHER_API_KEY (препоръчително) или OPENAI_API_KEY в .env или като environment променлива."
            )
...
"""
    ...
    async def _process_monthly_chunk(
        ...
        try:
            ...
            if has_partner_flag:
                assert partner_chart is not None  # guaranteed by has_partner_flag
